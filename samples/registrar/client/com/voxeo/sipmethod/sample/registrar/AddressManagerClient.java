...
public class AddressManagerClient {
  ...
  public void usage() {
    System.out.println("Usage: java -cp %JAXWSRILIB% com.voxeo.sipmethod.sample.registrar.AddressManagerClient <list | remove <uri>>");
    ...
    System.out.println("       -l<url>  -- service url (eg: -lhttp://localhost:8080/registrar/services/manager).");
  }
    ...
    try {
      ...
      if (args != null && args.length >= 1) {
        if (!opts.getURL().equalsIgnoreCase(opts.getDefaultURL())) {
          port = new AddressManagerService(new URL(opts.getURL() +  "?wsdl"), new QName("http://registrar.sample.sipmethod.voxeo.com/", "AddressManagerService")).getAddressManagerPort();
        }
        else {
